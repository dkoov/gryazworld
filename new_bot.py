...
async def sync_member_nick(discord_id: str, minecraft_nick: str):
    """Set the Discord display name for a single member to their Minecraft nick."""
    try:
        ...
        if member.nick != minecraft_nick:
            await member.edit(nick=minecraft_nick)
            log.info("Ник синхронизирован: %s -> %s", discord_id, minecraft_nick)
    except discord.Forbidden:
        log.warning("Нет прав изменить ник для %s", discord_id)
    except discord.NotFound:
        log.warning("Участник %s не найден на сервере", discord_id)
    except Exception as e:
        log.error("Ошибка синхронизации ника для %s: %s", discord_id, e)


async def sync_all_nicks():
    """Fetch all linked profiles from backend and sync Discord nicknames."""
    log.info("Запуск синхронизации всех ников...")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                BACKEND_URL + "/web/all_profiles",
                timeout=aiohttp.ClientTimeout(total=15),
            ) as resp:
                if resp.status != 200:
                    log.warning("all_profiles вернул статус %s", resp.status)
                    return
                profiles = await resp.json()
    except Exception as e:
        log.error("Не удалось получить all_profiles: %s", e)
        return

    log.info("Синхронизируем %d профилей...", len(profiles))
    for profile in profiles:
        discord_id = profile.get("discord_id")
        minecraft_nick = profile.get("minecraft_nick")
        if discord_id and minecraft_nick:
            await sync_member_nick(discord_id, minecraft_nick)
            await asyncio.sleep(0.5)  # rate limit protection
    log.info("Синхронизация ников завершена")


async def hourly_nick_sync_task():
    """Background task: sync all nicks every hour."""
    await client.wait_until_ready()
    while not client.is_closed():
        await sync_all_nicks()
...
async def handle_notify(request: web.Request) -> web.Response:
    ...
    try:
        if event_type == "nick_sync":
            discord_id = data.get("discord_id")
            minecraft_nick = data.get("minecraft_nick")
            if discord_id and minecraft_nick:
                asyncio.ensure_future(sync_member_nick(discord_id, minecraft_nick))
...
async def main():
    ...
    asyncio.ensure_future(hourly_nick_sync_task())
    async with client:
